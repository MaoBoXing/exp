...
def check(ip,port):
    ...
    try:
        ...
        if req.status_code == 200:                                                          #修改匹配返回代码
                ...
                logging.info(u"[+] {} find HIKVISION 视频编码设备接入网关 任意文件下载".format(ip))                                 #填写漏洞信息
                os._exit(1)
        else :
            logging.info(u"[-] {} do not found HIKVISION 视频编码设备接入网关 任意文件下载".format(ip))                             #填写漏洞信息
            os._exit(2)
    except Exception as e:
        logging.info(u"[*] {}".format(e))
        os._exit(4)
