...
def check(url):
    ...
    initial_request = exploit(url)
    if initial_request == "ERROR":
        result = False
        logging.info("The host does not respond as expected.")
        return(result)
...
def main(url):
    result = check(url)
    if result is True:
        logging.info( '[+] Status: '+url+'\tVulnerable!')
        os._exit(1)
    else:
        logging.info('[-] Status: '+url+'\tNot Affected.')
        os._exit(2)

if __name__ == '__main__':
    ip = sys.argv[1]
    port = sys.argv[2]
    url = ip+":"+port+'/orders/3'
    if '://' not in url:
        url = 'http://' + url
    url = url.strip('/')
    try:
