    ...
    try:
        ...
        if req.status_code == 200:
            if (b'flag=0' in req.text) :              #填写判断条件
                logging.info("[+] {} find Wayos_AC_Centralized_management_system_Default_weak_password".format(ip))                          #填写漏洞名
                os._exit(1)

        logging.info("[-] {} do not found Wayos_AC_Centralized_management_system_Default_weak_password".format(ip))                          #填写漏洞名
        os._exit(2)
    except Exception as e:
